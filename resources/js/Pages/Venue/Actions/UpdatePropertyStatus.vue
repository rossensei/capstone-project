...
const updateStatus = () => {
    form.post('/venues/show-properties/'+ props.venue.id +'/update-status', {
        onSuccess: () => {
            form.reset();
            emits('close-modal');
        }
...
<template>
    ...
    <div class="p-6 w-full rounded-lg max-h-">
        <h1 class="text-xl text-gray-700 font-semibold mb-2">Update Status for "{{ property.name }}"</h1>
        <form @submit.prevent="updateStatus">
            <div class="block mb-4">
                <label for="venues" class="text-sm text-gray-600 font-semibold">Select Venue</label>
                <select v-model="form.status" id="venues" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm w-full text-sm">
