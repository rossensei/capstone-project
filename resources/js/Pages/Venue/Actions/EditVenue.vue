...
const submit = () => {
    form.patch('/venues/update-details/' + props.venueId, {
...
onMounted(() => {
    ...
    axios.get('http://127.0.0.1:8000/api/venues/' + props.venueId)
    .then((response) => {
        // console.log(response)
        form.user_id = response.data.user_id;
        form.venue_name = response.data.venue_name;
        form.building = response.data.building;
    })
    .then(() => {
        isLoaded.value = true;
    })
...
<template>
    ...
    <div v-else class="bg-white w-full shadow p-6 rounded-lg">
        <h1 class="text-xl text-gray-700 font-semibold mb-2">Edit Venue Details</h1>
