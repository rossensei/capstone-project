...
const confirm = () => {
    form.post('/venues/show-properties/'+ props.venue.id +'/move-property', {
        onSuccess: () => {
            form.reset();
            emits('close-modal');
        }
...
<template>
    ...
    <div v-else class="p-6 w-full rounded-lg max-h-">
        <h1 class="text-xl text-gray-700 font-semibold mb-2">Move "{{ property.name }}" to</h1>
        <form @submit.prevent="confirm">
            <div class="block mb-4">
                <label for="venues" class="text-sm text-gray-600 font-semibold">Select Venue</label>
                <select v-model="form.venueId" id="venues" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm w-full text-sm">
            ...
            <div class="flex items-center justify-end space-x-2">
                ...
                <button type="submit" class="px-4 py-2 text-sm rounded-md bg-[#4e73df] hover:bg-[#4e72dfc0] text-white ml-2" :class="{ 'bg-[#4e72dfc0]' : form.processing }" :disabled="form.processing">
                    ...
                    <span v-if="!form.processing">Move</span>
                </button>
            </div>
        </form>
    </div>
