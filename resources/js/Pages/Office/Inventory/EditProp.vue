...
onMounted(() => {
    axios.get('http://127.0.0.1:8000/api/properties/' + props.propertyId)
        .then((response) => {
            form.name = response.data.name;
            form.description = response.data.description;
            form.date_acquired = response.data.date_acquired;
        }).finally(() => {
            isLoaded.value = true;
        })
})
...
const submit = () => {
    form.patch('/admin/offices/' + props.office.id + '/inventory/' + props.propertyId, {
        onSuccess: () => {
            emits('close-modal') ;
            form.reset();
        }
...
<template>
    ...
    <div v-if="!isLoaded" class="w-full p-6 animate-pulse">
        ...
        <div class="mb-4 bg-gray-200 rounded-full h-8 w-full"></div>
        <div class="bg-gray-200 rounded-full h-8 min-w-[150px] flex-1 mb-4"></div>
        <div class="bg-gray-200 rounded-full h-8 min-w-[150px] flex-1 mb-4"></div>

        <div class="flex justify-end items-center">
            <div class="h-8 w-[120px] bg-gray-200 rounded-full"></div>
            <div class="h-8 w-[120px] bg-gray-300 rounded-full ml-3"></div>
        </div>
    </div>
