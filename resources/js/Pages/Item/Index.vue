...
import { PencilIcon, PlusIcon, MagnifyingGlassIcon, ChevronUpIcon, ArrowPathIcon, PlusCircleIcon } from '@heroicons/vue/24/outline';
...
watch(() => params, debounce(() => {
    ...
    router.get('/items', newParams, { preserveState: true, replace: true });
}, 300), {
    deep: true
})
...
<template>
    ...
    <AuthenticatedLayout>
        <div class="py-4">
            <div class="w-full sm:px-6 lg:px-8">
                ...
                <div class="flex items-center justify-between mb-4">
                    <h2 class="font-semibold text-3xl text-gray-700 leading-tight">Items</h2>
                    <button type="button" @click="createItem" class="flex items-center px-3 py-2 rounded-lg bg-[#4e73df] hover:bg-[#4e72dfc0] text-white font-medium">
                ...
                <div class="relative overflow-x-auto bg-white shadow rounded-lg sm:py-4 lg:py-6">
                    ...
                    <table class="w-full text-sm text-left text-gray-500">
                        ...
                        <tbody>
                            <tr class="border-b border-t bg-white" v-for="item in items.data" :key="item.id" :class="{'border-l-4 border-l-blue-600' : selectedItems.includes(item.id)}">
                                ...
                                <td class="px-6 py-3 text-center">
                                    {{ item.qty_stock }} {{ item.unit_abbreviation }} 
                                </td>
                                <!-- <td class="px-6 py-3 text-center">
                                </td> -->
                                <td class="px-6 py-3 text-center">
        ...
        <Modal :show="showModal" maxWidth="xl" @close="closeModal">
            <CreateItem v-if="actionType === 'create'" @close-modal="closeModal" />
            <EditItem v-else-if="actionType === 'edit'" :itemId="selectedItemForEdit" @close-modal="closeModal" />
