...
onMounted(() => {
  axios.get('http://127.0.0.1:8000/api/personnels/' + props.personnel.id)
    .then((response) => {
...
const submit = () => {
  form.patch('/personnels/update-details/' + props.personnel.id, {
      onSuccess: () => {
        emits('close-modal');
        form.reset();
      }
...
<template>
    ...
    <div v-else class="p-6 w-full">
        <h4 class="text-md font-semibold text-gray-600">Edit Personnel</h4>

        <form @submit.prevent="submit">
            <div class="mt-3 mb-4">
                <InputLabel for="name" value="Personnel Name" />
                <TextInput v-model="form.name" id="name" class="w-full text-sm" :class="{ 'border-red-600' : form.errors.name }" placeholder="Full name" />
            ...
            <div class="mt-3 mb-4">
                ...
                :class="{'border-red-600' : form.errors.office_id}" class="w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm text-sm"
                >
            ...
            <div class="flex justify-end items-center space-x-2">
                ...
                <button type="submit" class="inline-flex items-center px-4 py-2 text-sm font-medium rounded-lg bg-[#4e73df] hover:bg-[#4e72dfc0] text-white" :disabled="form.processing">
